...
# This script is a meld of automongobackup detailed below 
# and a meteor dump script written by @olizilla On 2013-03-20.
#
#
...
#=====================================================================
# meteor mongo specific
METEOR_DOMAIN="protobrew.meteor.com"
PASSWORD="12ezferu"
#=====================================================================
...
dbdump () {
    ...
    export TMP_FILE
    export METEOR_DOMAIN
    export PASSWORD

    # Use Expect to respond to the password prompty while getting
    # the mongo url for your meteor app
    ...
    mongodump -u $MONGO_USER -h $MONGO_DOMAIN -db $MONGO_DB -p "${MONGO_PASSWORD}" --out=$1
    [ -e "$1" ] && return 0
...
if [ "$MONGO_DOMAIN" = "localhost" -o "$MONGO_DOMAIN" = "127.0.0.1" ]; then
    HOST=`hostname`
    if [ "$SOCKET" ]; then
